...
window.addEventListener("load", function() {
    ...
    leftWin = chrome.app.window.get('timer_window').getBounds().left;
    widthWin = chrome.app.window.get('timer_window').getBounds().width;
    libs.window.data.this.moveTo(leftWin + widthWin, topWin);
    //POSITION RELATIVE TO TIMER WINDOW
    chrome.app.window.get('timer_window').onBoundsChanged.addListener(function() {
        left = chrome.app.window.get('timer_window').getBounds().left;
        width = chrome.app.window.get('timer_window').getBounds().width;
        console.log("LOLOLOLOLOL: " + left);
        libs.window.data.this.moveTo(left + width, chrome.app.window.get('timer_window').getBounds().top);
    });
    setTimeout(function() {
        libs.msg.send.libs.data("timer_window", "timer", "status", function(status) {
            libs.msg.send.libs.call("timer_window", "window.popup.timerCollapse", [status]);
